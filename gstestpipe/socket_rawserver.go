		...
		for i := 0; i < 32; i++ {
			...
			go func() {
				...
				for !ss.close.Load() {
					...
					checkError_exit(err)
					connaobj := gsnetconn.NewGSTNetConn(conna)

					ss.lock_connListIn.Lock()
					ss.connListIn = append(ss.connListIn, connaobj)
					ss.lock_connListIn.Unlock()
					ss.connList <- connaobj
				}
			}()
...
type RawServerSocketEcho struct {
	...
	count_list_echo_bytes *gsmap.GsMapStr_Str
}
...
func NewRawServerSocketEcho_RandAddr() *RawServerSocketEcho {
	return &RawServerSocketEcho{RawServerSocket: RawServerSocket{connList: make(chan net.Conn, 10240),
		serverAddr: gstunnellib.GetNetLocalRDPort()},
		count_list_echo_bytes: gsmap.NewGSMapStr_Str(),
	}
}
