...
func inTest_client_socket(t *testing.T, mt_mode bool, mbNum int) {
	...
	g_networkTimeout = time.Second * 2

	gstServer := gstestpipe.NewGstRSAServerSocketEcho_RandAddr_DefaultKey()
	...
	gstClientT := newGstClient(listenAddr, gstServer.GetServerAddr())
	defer gstClientT.close()
	go gstClientT.run()
