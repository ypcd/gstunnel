...
func inTest_client_NetPipe(t *testing.T, mt_mode bool) {
	...
	rawClient := gstestpipe.NewRawClientNone()

	//key1 := gsrand.GetRDBytes(gsbase.G_AesKeyLen)
	//key1 := g_key
	serverKey := gsrsa.NewRSAObjFromBase64([]byte(gsbase.G_DefaultRSAKeyPrivate))
	gstServer := gstestpipe.NewGSTRSAServerImp(string(g_key), serverKey)
