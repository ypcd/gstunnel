...
func aestpack() {
	...
	fbuf := gsrand.GetRDBytes(50 * 1024)

	a1 := NewGsPack(string(gsrand.GetRDBytes(gsbase.G_AesKeyLen)))
	tmp := a1.Packing(fbuf)
	outbuf, _ := a1.Unpack(tmp)
...
func Test_AesEncryDecry(t *testing.T) {
	d1 := gsrand.GetRDBytes(15834)
	key1 := gsrand.GetRDBytes(gsbase.G_AesKeyLen)
	key2 := gsrand.GetRDBytes(gsbase.G_AesKeyLen)
...
func Test_hex_data(t *testing.T) {
	v1 := gsrand.GetRDInt64()
	hx1 := hex.EncodeToString(gsrand.Int64ToBytes(v1))
	be1 := base64.StdEncoding.EncodeToString(gsrand.Int64ToBytes(v1))
	v2, _ := hex.DecodeString(hx1)
	t.Log(v1, hx1, be1, bytes.Equal(gsrand.Int64ToBytes(v1), v2))

	vv1 := gsrand.GetRDBytes(2048)
...
func Test_pack_type_size(t *testing.T) {
	ap1 := NewGsPack(string(gsrand.GetRDBytes(gsbase.G_AesKeyLen)))
	t.Log(len(ap1.Packing([]byte{})))
	t.Log(len(ap1.ChangeCryKey()))
	t.Log(len(ap1.PackVersion()))
}

func Test_gspack_pack_unpack_run_loop(t *testing.T) {
	pn := NewGsPack(gsbase.G_AesKeyDefault)
	rawdata := gsrand.GetRDBytes(int(gsrand.GetRDInt_max(50 * 1024)))
...
func Test_GetRDKeyBase64(t *testing.T) {
	key1str := gsrand.GetRDKeyBase64(gsbase.G_AesKeyLen)
